...
public class AIService
{
	...
	private string GetDictionaryHints(string text)
	{
		...
		foreach (var w in words)
		{
			...
			if (_localDataSetsProvider.SpecializedDictionary.TryGetValue(clean, out var match))
			{
	...
	private WordBreakdownItem? ParseGenericWordBreakdown(string word)
	{
		...
		if (_localDataSetsProvider.SpecializedDictionary.TryGetValue(cleanWord, out var entry))
		{
		...
		if (_localDataSetsProvider.CommonDictionary.TryGetValue(cleanWord, out var commonEntry))
		{
			return new WordBreakdownItem(
				Word: cleanWord,
	...
	private ScriptureAnalyzeResponse? TryMatchingScriptureArchive(string text)
	{
		...
		{
			if (_localDataSetsProvider.PopularArchive.TryGetValue("karma", out var match)) return match;
		}
		...
		{
			if (_localDataSetsProvider.PopularArchive.TryGetValue("yada", out var match)) return match;
		}
		...
		{
			if (_localDataSetsProvider.PopularArchive.TryGetValue("tryambakam", out var match)) return match;
		}
		...
		{
			if (_localDataSetsProvider.PopularArchive.TryGetValue("om bhur", out var match)) return match;
		}
		...
		{
			if (_localDataSetsProvider.PopularArchive.TryGetValue("saha na", out var match)) return match;
		}
		...
		{
			if (_localDataSetsProvider.PopularArchive.TryGetValue("isha vasyam", out var match)) return match;
		}
	...
	private TranslationResponse GenerateTranslateFallback(string text, string targetLang)
	{
		...
		if (_localDataSetsProvider.CommonDictionary.TryGetValue(cleanInput, out var dictMatch))
		{
		...
		else
		{
			var words = tokens.Select(tok =>
			{
				...
				if (_localDataSetsProvider.CommonDictionary.TryGetValue(wordClean, out var m))
				{
