...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        if (!Directory.Exists(webClientDist))
        {
            webClientDist = Path.Combine(contentRoot, "..", "..", "..", "..", "Sanskrit-Quest-Scripture-Client", "dist");
        }
        if (!Directory.Exists(webClientDist))
        {
        ...
        if (!Directory.Exists(webClientDist))
        {
            Directory.CreateDirectory(webClientDist);
        }
        Console.WriteLine($"[Web.Api] React Static Web Root Path: {webClientDist}");
        builder.Environment.WebRootPath = webClientDist;
        builder.Environment.WebRootFileProvider = new Microsoft.Extensions.FileProviders.PhysicalFileProvider(webClientDist);
